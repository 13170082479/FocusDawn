...
- 目标页支持单张目标卡片保存周目标，也支持保存全部目标设置
- 周目标自定义输入支持小时/分钟格式，例如 `0.5`、`2.5`、`30分钟`、`90 min`
- 目标页支持按默认、周目标、已完成、完成率、剩余时间排序
...
- 目标页支持周目标单卡保存、全部保存和排序
- 游戏/平台提醒弹窗
- 目标完成 toast
- 托盘图标和托盘菜单
- 设置页保存每日目标、黑名单、强制关闭、自启动
- 设置页黑名单管理：扫描当前进程、推荐可疑娱乐进程、手动添加、删除
...
- 设置页已从纯文本黑名单升级为进程管理器式 UI
- 扫描结果会将 Steam、WeGame、shipping、launcher、game 等关键词进程排在前面
- explorer.exe、svchost.exe、FocusDawn、python 等受保护进程禁止加入黑名单
...
- 周目标可以在每张目标卡片上单独保存，避免滚到底部才能保存
- 自定义周目标支持输入小时或分钟，示例：`0.5`、`2.5`、`30分钟`、`90 min`
- 目标列表支持排序：默认、周目标高优先、已完成高优先、完成率高优先、剩余时间少优先
- 目标可编辑名称、图标、颜色
- 目标可归档，历史记录不删除
...
- 完整编辑目标名称、图标、颜色
- 删除目标时归档，不直接删除历史记录
- 目标管理页体验优化：目标卡片、周目标输入规范化、单卡保存、排序
...
5. 修复 UI/日志中的历史乱码字符串。
6. 将黑名单继续升级为分组管理：游戏平台、游戏、附属进程。
7. 给 Steam/WeGame 关闭策略增加更细控制，例如是否关闭 webhelper/service。
8. 目标页支持拖拽调整重点目标顺序，决定首页“本周计划”前三个展示对象。
...
下一阶段重点是把“目标系统”从好用推进到可复盘：补齐按目标筛选趋势图、历史周目标明细、归档目标恢复和轻量成就系统，让 FocusDawn 更稳定地回答“时间花在了哪些重要方向上”。
