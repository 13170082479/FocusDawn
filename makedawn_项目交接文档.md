...
- 一级「目标」模块支持卡片式目标列表、新建目标弹窗、设置本周目标时间
...
- 一级「目标」模块：“目标管理与本周计划”，卡片式展示目标、进度、剩余/超额状态
- 游戏/平台提醒弹窗
- 目标完成 toast
- 托盘图标和托盘菜单
...
- 连续达标天数目前基于传入分析范围计算，严格来说可能受 7 天/30 天/全部筛选影响。
- 首页右侧“当前状态”仍使用 CTk 控件叠加结构，如果以后恢复背景图，仍可能遇到黑框问题。
- 周目标目前只支持当前周编辑，历史周计划编辑放第二阶段。
- 目标管理当前只支持新增和设置本周目标，完整编辑/归档放第二阶段。

## 12. 建议下一步

优先级建议：

1. 完整目标编辑：名称、图标、颜色、归档。
2. 历史每周完成率和周报列表。
3. 趋势图支持按目标筛选。
4. 增加“扫描当前娱乐进程”按钮，在设置页显示当前命中的进程。
5. 修复 UI/日志中的乱码字符串。
6. 将黑名单从纯文本升级为分组管理：游戏平台、游戏、附属进程。
7. 给 Steam/WeGame 关闭策略增加更细控制，例如是否关闭 webhelper/service。
8. 修正连续达标天数口径，使其不受分析范围筛选影响。
